...
async function setVotingDuration(newDuration: number) {
  await log(`setVotingDuration ${newDuration}`);
  try {
    const tx = await voteProgram.methods
      .setVotingDuration(new anchor.BN(newDuration))
      .accounts({
        roundAccount,
        config,
    ...
    await logTx(`Voting duration updated: ${tx}`);
  } catch (error) {
    await log(`Error setting voting duration ${error}`);
    throw error;
  }
}
...
async function main() {
  ...
  await setVotingDuration(votingDurationSeconds);
}
